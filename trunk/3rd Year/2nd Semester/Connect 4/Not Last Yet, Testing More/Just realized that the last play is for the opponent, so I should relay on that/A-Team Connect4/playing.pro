...
%	This is called the Lame (or maybe the lamist Clause Ever), I force the computer to put it's first in the middle !!!, because my algorithm (until time of writing this line), isn't strong enough to understand this.
	aB(CBoard,Player,NewCBoard):-
		...
		% If I am empty board
		Board = EmptyBoard,
		% And I am ex (Computer)
		Player = ex,
		
		% Go Set my choice to the middle, shame on me
		setSquareValue(cbrd(Board,sqr("d",1)),Player,NewBoard),
		NewCBoard = cbrd(NewBoard,sqr("d",1)),	!.
...
%			Evaluate Board
	...
	evaluateBoard(CBoard,Player,Return):-%check for forks for me
		extractCBoard(CBoard,Board,_),
		getNextPlays(Board,SqrList),
		doesFork(Board,SqrList,Player),
		playerValue(Player,Value),
		Return = Value * 1000	,!.
	evaluateBoard(CBoard,Player,Return):-%check for forks for the opponent
		extractCBoard(CBoard,Board,_),
		opponentPlayer(Player,Opponent),
		getNextPlays(Board,SqrList),
		...
		playerValue(Opponent,OpponentValue),
		Return = OpponentValue * 1000	,	!.
	evaluateBoard(CBoard,Player,Return):-%this play will make a win for me
		extractCBoard(CBoard,Board,Square),
		...
		playerValue(Player,Value),
		Return = Value * 900,	!.
	evaluateBoard(CBoard,Player,Return):-%this play will make a win for the opponent
		extractCBoard(CBoard,Board,Square),
		opponentPlayer(Player,Opponent),
		adj(Square,up,SquareAbove),
	...
	evaluateBoard(CBoard,Player,Return):-%if last opponent play did block my win, it is bad, or like if I did play here, i would win
		extractCBoard(CBoard,_,Square),
		opponentPlayer(Player,Opponent),
		
		setSquareValue(CBoard,Player,NewBoard),
		didWin(cbrd(NewBoard,Square),Player),
