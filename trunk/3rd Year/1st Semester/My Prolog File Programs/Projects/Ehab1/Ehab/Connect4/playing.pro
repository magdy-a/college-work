...
clauses
	...
	aB(CBoard,ex,NewCBoard):-
		...
		alphaBeta(CBoard,MinusInfinity,PlusInfinity,ex,2,_,NewCBoard),%default depth is 3
		!.
	aB(CBoard,oh,NewCBoard):-
		minusInfinity(MinusInfinity),
		plusInfinity(PlusInfinity),
		alphaBeta(CBoard,MinusInfinity,PlusInfinity,oh,3,_,NewCBoard).%default depth is 2
	
	
	alphaBeta(CBoard,_,_,ex,0,Number,CBoard):-
		didWin(CBoard,ex),
		evaluationFunction(CBoard,ex,win,Number),
		write("Halaloya ex wins!"),	nl,	!.
	alphaBeta(CBoard,_,_,oh,0,Number,CBoard):-
		didWin(CBoard,oh),
		evaluationFunction(CBoard,oh,win,Number),
		write("Halaloya oh wins!"),	nl,	!.
		
	alphaBeta(CBoard,_,_,ex,_,Number,CBoard):-
		didWin(CBoard,oh),
		evaluationFunction(CBoard,oh,win,Number),
		write("Halaloya Oh wins!"),	nl,	!.
	alphaBeta(CBoard,_,_,oh,_,Number,CBoard):-
		didWin(CBoard,ex),
		evaluationFunction(CBoard,ex,win,Number),
		write("Halaloya Ex wins!"),	nl,	!.
...
%		evaluationFunction(cbrd(Board,LastPlay),Player,check,Heuristic),
	...
	evaluate_Board(Board,ex,SqrList,-1000):-
		doesFork(Board,SqrList,oh),	!.
		
	evaluate_Board(Board,oh,SqrList,1000):-
		doesFork(Board,SqrList,ex),	!.
		
	evaluate_Board(Board,_,SqrList,Number):-
		evaluateBoardAux(Board,SqrList,Ex),
		evaluateBoardAux(Board,SqrList,Oh),
		Number = Ex + Oh,	!.
		
	evaluateBoardAux(_,[],0):-	!.
	evaluateBoardAux(Board,[Head|Tail],Number):-
	
		setSquareValue(cbrd(Board,Head),ex,NewBoard),
		evaluationFunction(cbrd(NewBoard,Head),ex,check,Ex),
	...
	evaluationFunction(cbrd(Board,Square),ex,check,-500):-%this play will make the opnent block a my win which is bad ( block X win next time)
...
*/
	...
	evaluationFunction(cbrd(Board,Square),oh,check,500):-%this play will make the opnent block a my win which is bad ( block X win next time)
		adj(Square,up,NextSquare),
		setSquareValue(cbrd(Board,NextSquare),oh,NewBoard),
		didWin(cbrd(NewBoard,NextSquare),oh),	!.
