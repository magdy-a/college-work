...
namespace MapDataEditor
{
    ...
    class SpriteSheetMaker
    {
        ...
        {
            Point targetPixel = new Point(x * spriteSize.Height, y * spriteSize.Height);
        ...
        {
            //will not work right in case of Ground Object !!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            Bitmap[] SheetBitmap = new Bitmap[spritesCount];
            int Counter = 0;
            for (int i = 0; i < sheetSize.Width; i++)
            {
                for (int j = 0; j < sheetSize.Height; j++)
                {
                    SheetBitmap[Counter++] = GetSprite(i, j);
                }
            }
            return SheetBitmap;
        }
