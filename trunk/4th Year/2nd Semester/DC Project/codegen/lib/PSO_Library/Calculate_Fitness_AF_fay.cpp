...
real_T Calculate_Fitness_AF_fay(const real_T p[48], real_T fay)
{
  ...
  eml_scalar_rem90(90.0, &AF1_re, &AF1_im);
  if (AF1_re == 0.0) {
    AF1_im = sin(AF1_im);
  } else if (AF1_re == 1.0) {
    AF1_im = cos(AF1_im);
  ...
  return rt_hypotd(fabs(AF1_re), fabs(AF1_im));
}

/* End of code generation (Calculate_Fitness_AF_fay.cpp) */
