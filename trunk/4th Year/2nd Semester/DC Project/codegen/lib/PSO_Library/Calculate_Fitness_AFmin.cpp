...
real_T Calculate_Fitness_AFmin(const real_T p[48])
{
  ...
  eml_scalar_rem90(90.0, &AF4_re, &AF4_im);
  if (AF4_re == 0.0) {
    AF4_im = sin(AF4_im);
  } else if (AF4_re == 1.0) {
    AF4_im = cos(AF4_im);
  ...
  return -(((rt_hypotd(fabs(AF1_re), fabs(AF1_im)) + rt_hypotd(fabs(AF2_re),
              fabs(AF2_im))) + rt_hypotd(fabs(AF3_re), fabs(AF3_im))) -
           rt_hypotd(fabs(AF4_re), fabs(AF4_im)));
}

/* End of code generation (Calculate_Fitness_AFmin.cpp) */
