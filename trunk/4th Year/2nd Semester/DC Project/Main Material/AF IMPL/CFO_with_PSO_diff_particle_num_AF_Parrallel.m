...
%-----------intialize probes on axis and put best probe from CFO in first probe of PSO--------------
% NNN=NParticles/Nd;
% Ratio=(maxdimension-mindimension)/(NNN-1);
% pp=1;
% for i=1:Nd
...
%intialize particles with best propes in cfo
R(1:NParticles,:,1)=get_best_propes(R,M,Nd,NParticles);
oldpfitness=zeros(1,NParticles);
pbest=zeros(NParticles,Nd);
[iterbest,pbest,oldpfitness,indx,gfitness]=calculate_pbest_obj_AF(R(:,:,1),pbest,oldpfitness);
gbest=iterbest;
...
title('radiation after pso with Nelder Mead');
