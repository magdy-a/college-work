...
CvMat* lsq_homog( CvPoint2D64f* pts, CvPoint2D64f* mpts, int n )
{
	...
	// TODO Test For Loop, remove it and use the one after it
	...
	/*for(i = 0 ; i < 2*n ; i ++)
		...
		for(j = 0 ; j < 8 ; j ++)
			{
			dTest = cvmGet(A,i,j);
			fprintf(stderr,"%d :: ", cvRound( dTest));
			}

		fprintf(stderr,"  :: %d\n", cvRound(cvmGet(B,i,0)));
		}*/

	// TODO End Clear this test code late
