...
%	----------------<<<<<<< 	Use backtrack which returns a list of symbols from start till the sent SearchNode in the SearchTree 	 -----------------<<<<<<<<<	
	nondeterm backtrack(searchNode,snList,sList).
	nondeterm backtrackAux(searchNode,snList,sList,sList).
	
	nondeterm backtrackSearchList(searchNode,snList,snList).
	nondeterm backtrackSearchListAux(searchNode,snList,snList,snList).
	
	nondeterm printSearchList(snList).
	nondeterm printSearchNames(snList).
...
%			at goal ... print the (path) of every node in the search node, because in a graph a single node might be reached by 2 ways.

	bestFirstSearch(Symbol,uniform):-
		rootNode(Name),
		node(Name,H),
		bestFS(Symbol,[sn(Name,0,0,0,H,0)],uniform).
	...
	bestFS(_Goal,[sn(_Goal,B,Ex,G,H,M)|Tail],uniform):-
		...
		backtrackSearchList(sn(_Goal,B,NewEx,G,H,M),[sn(_Goal,B,NewEx,G,H,M)|Tail],Path),		
...
%			if Node and first Element are in fringe  && Node's Maths less than the first Node ... so insert First
	...
	backtrackAux(sn(Name,0,_,_,_,_),_,[Name|Aux],Aux).
	backtrackAux(sn(Name,B,_,_,_,_),List,Solution,Aux):-
		B <> 0,
		getSearchListElement(B,List,NewNode),
		backtrackAux(NewNode,List,Solution,[Name|Aux]).	
		
	backtrackSearchList(Node,List,Solution):-
		backtrackSearchListAux(Node,List,Solution,[]).
	
	backtrackSearchListAux(sn(Name,B,Ex,G,H,M),_,[sn(Name,B,Ex,G,H,M)|Aux],Aux):-
		B = 0.
	backtrackSearchListAux(sn(Name,B,Ex,G,H,M),List,Solution,Aux):-
		B <> 0,
		getSearchListElement(B,List,NewNode),
		backtrackSearchListAux(NewNode,List,Solution,[sn(Name,B,Ex,G,H,M)|Aux]).
		
	printSearchList([]).
	printSearchList([sn(Name,B,Ex,G,H,M)|Tail]):-
		Ex <> 0,
		write("Name : ", Name,", backtrack : ",B,", Expansion Number : ",Ex,", G : ",G,", H : ",H," and M : ",M," ...  "),
		nl,
		printSearchList(Tail).
	printSearchList([sn(Name,B,Ex,G,H,M)|Tail]):-
		Ex = 0,
		write("Name : ", Name,", backtrack : ",B,", Expansion Number : Fringe",", G : ",G,", H : ",H," and M : ",M," ...  "),
		nl,
		printSearchList(Tail).
	
	printSearchNames([]).
	printSearchNames([sn(Name,_,_,_,_,_)|Tail]):-
		write(Name," "),	
		printSearchNames(Tail).
...
Goal
	...
	bestFirstSearch("G",aStar).
	
%	bestFirstSearch("G",greedy).
%	bestFirstSearch("G",uniform).

%	backtrack(sn("C",3,3,3,7,10),[sn("S",0,1,0,5,5),sn("A",1,2,0,5,5),sn("C",1,2,4,8,12),sn("B",2,3,3,7,10)],What).
