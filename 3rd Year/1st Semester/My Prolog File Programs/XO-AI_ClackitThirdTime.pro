...
%			A.I
	...
	alphaBetaAux(bList,integer,integer,integer,board,turn,integer,board).
...
%	alphaBeta(Board,_,_,_,Heuristic,Board):-	isTerminal(Board),	/*write("Draw"),nl,nl,*/	Heuristic = 0,	!.
	alphaBeta(Board,_,_,_,Heuristic,Board):-	isTerminal(Board),	write("Draw"),nl,nl,	Heuristic = 0,	!.
%					maxX
	alphaBeta(Board,Alpha,Beta,maxX,Heuristic,NewBoard):-	getChildren(Board,"X",[Head|Tail]),write("Children"),nl,printBoardList([Head|Tail]),write("EndChildren"),nl,	alphaBetaAux([Head|Tail],Alpha,Beta,Alpha,Head,maxX,Heuristic,NewBoard),!.
%					minO

	alphaBeta(Board,Alpha,Beta,minO,Heuristic,NewBoard):-	getChildren(Board,"O",[Head|Tail]),write("Children"),nl,printBoardList([Head|Tail]),write("EndChildren"),nl,	alphaBetaAux([Head|Tail],Alpha,Beta,Beta,Head,minO,Heuristic,NewBoard),!.
	
		 
%				   AlphaBetaAux

%					maxX
	alphaBetaAux([],Alpha,_,_,NewBoard,maxX,Alpha,NewBoard):-	!.
	alphaBetaAux([Board|Tail],Alpha,Beta,Value,TmpBoard_1,maxX,NewAlpha,NewBoard):-
		Beta > Alpha,
		printBoard(Board),
		alphaBeta(Board,Alpha,Beta,minO,Heuristic,_),	max(Alpha,Heuristic,NewAlphaAux),
		maxWithBoard(Value,TmpBoard_1,Heuristic,Board,NewValue,NewTmpBoard),	alphaBetaAux(Tail,NewAlphaAux,Beta,NewValue,NewTmpBoard,maxX,NewAlpha,NewBoard),!.
	alphaBetaAux(List,Alpha,Beta,_,TmpBoard,maxX,Alpha,TmpBoard):-	Beta <= Alpha, write("PrunnedList"),nl,printBoardList(List),write("EndPrunnedList"),nl,	!.
%					minO
	alphaBetaAux([],_,Beta,_,NewBoard,minO,Beta,NewBoard):-	!.
	alphaBetaAux([Board|Tail],Alpha,Beta,Value,TmpBoard_1,minO,NewBeta,NewBoard):-
		Beta > Alpha,
		printBoard(Board),
		alphaBeta(Board,Alpha,Beta,maxX,Heuristic,_),	min(Beta,Heuristic,NewBetaAux),
		minWithBoard(Value,TmpBoard_1,Heuristic,Board,NewValue,NewTmpBoard),	alphaBetaAux(Tail,Alpha,NewBetaAux,NewValue,NewTmpBoard,minO,NewBeta,NewBoard),!.
	alphaBetaAux(List,Alpha,Beta,_,TmpBoard,minO,Beta,TmpBoard):-	Beta <= Alpha,write("PrunnedList"),nl,printBoardList(List),write("EndPrunnedList"),nl,	!.
	...
	getPlay(Return,Player):-
		write("It's ",Player,"'s turn ... enter value between 1 and 9 ..."),nl,
		readint(Number),
		Number >= 1,
		Number <= 9,
		Return = Number - 1,
