...
%			AlphaBeta
	...
	alphaBeta(CBoard,_,_,Player,_,Return,CBoard):-%win for my opponent ( depth isn't zero ... so the lastPlay in CBoard is really to opponent player's, return 1000 * opponentValue ( just Checking the last play )
		opponentPlayer(Player,Opponent),
		didWin(CBoard,Opponent),
...
%			End of didWin
	...
	countSpaces(brd([Head|Tail]),NewNumber):-%head is Spaces ... reutrn tail  + 1
		...
		NewNumber = Number + 1,	!.
	countSpaces(brd([Head|Tail]),Number):-%Head isn't Space ... return tail
		countSpaces(brd(Tail),Number).
