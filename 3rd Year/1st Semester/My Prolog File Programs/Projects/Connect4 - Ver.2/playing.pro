...
clauses
	...
	play.%else ... oh but input isn't valid !
	...
	alphaBeta(CBoard,_,_,Player,_,Return,CBoard):-%win for my opponent
		opponentPlayer(Player,Opponent),
		didWin(CBoard,Opponent),
		playerValue(Opponent,OpponentValue),
		Return = OpponentValue * 1000,	!.
		
	alphaBeta(CBoard,_,_,_,_,0,CBoard):-%draw
...
%			Evaluate Board
	...
	getNextPlaysAux(_,Square,[]):-
		adj(Square,right,NextSquare),
		Square = NextSquare,	!.	getNextPlaysAux(Board,Square,[NewSquare|Tail]):-
		getSquareValue(cbrd(Board,Square),none),
		adj(Square,right,NextSquare),
		setChoice(cbrd(Board,Square),none,cbrd(_,NewSquare)),
...
/*
  	assert(currentCBoard(cbrd(brd([" ","O","X","X","X","X","X",
		                       "O","O","X","X","X","X","X",
     		                       "X","X","X","X","X","X","X",
		                       "X","X","X","O","O","X","X",
		                       "X","X","X","X","X","X","X",
		                       "X","X","X","O","O","X","X"]),sqr("a",1)))),
*/
	assert(currentSituation(playing)),
	assert(inputValidation(notValid)),
	assert(userInput(sqr("d",6))),
	assert(currentPlayer(oh)),%as tmp
  
	dlg_ListSelect ("Choose who plays first",["Play first","Play second"],0,_,SelectedIndex),
...
%BEGIN Playing, e_KeyDown
	win_playing_eh(_Win,e_KeyDown(k_up,_),0):-!,
  	
  	
  		retract(currentCBoard(_)),
	  	emptyBoard(EmptyBoard),
		asserta(currentCBoard(cbrd(EmptyBoard,sqr("a",1)))),
	
		dlg_ListSelect ("Choose who plays first",["Play first","Play second"],0,_,Player),
