...
clauses
	...
	% for the sake of performance, and memory saving:
	% shouldn't I make the Checking function (double valued), that is checking for the same operation for both players
	% and it will lead in changing the structure of the code, but it'll save some
	% like if I am checking for didWin condition for me, then the next for the opponent
	% I could like, read operate set values in the database, then go the backtracking with the values i created once
	% summary: predicates like didWin & doesFork, I call it twice every node check, I want to reduce them to only one, by making the predicate check for both in the same time (as it is the same loop), and it puts the results in the database, so i can use it as I want, functions may affect (didWin, doesFork ... etc)
	...
	aB(CBoard,Player,NewCBoard):-%just call alphaBeta
		...
		alphaBeta(CBoard,MinusInfinity,PlusInfinity,Player,4,_,NewCBoard),	!.%The depth here is 4, it is even and it must be always.
	
%			AlphaBeta

/*
	alphaBeta(CBoard,_,_,Player,_,Return,CBoard):-%here the opponent is acutally me the human ( the last play was the opponent )
		opponentPlayer(Player,Opponent),
		didWin(CBoard,Opponent),
		...
		*/
		...
		%Replacment for the last commented predicate
		% I think this would never ever be true, cause the lastSquare, is for the opponent, and you check for the player, how could the last player's square makes you win ??
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	alphaBeta(CBoard,_,_,Player,_,Return,CBoard):-%this play will block Opponent win ((Last Play))
		...
		Return = Value * 900,	!.
	alphaBeta(CBoard,_,_,Player,_,Return,CBoard):-%this play will allow blocking my own win after that (Just Added it)
		extractCBoard(CBoard,Board,Square),
		opponentPlayer(Player,Opponent),
		adj(Square,up,SquareAbove),
		setSquareValue(cbrd(Board,SquareAbove),Player,NewBoard),
		didWin(cbrd(NewBoard,SquareAbove),Player),
		playerValue(Opponent,OpponentValue),
	...
	alphaBeta(CBoard,_,_,Player,_,Return,CBoard):-%this play will be good next time for Opponent ((Last Play))
		extractCBoard(CBoard,Board,Square),
		opponentPlayer(Player,Opponent),
	...
	alphaBeta(CBoard,_,_,Player,_,Return,CBoard):-
		extractCBoard(CBoard,Board,_),
		%should I remove last play from the Board first ?
		getNextPlays(Board,SqrList),
		doesFork(Board,SqrList,Player),
		playerValue(Player,Value),
		Return = Value * 800	,!.
...
/*
	evaluateBoard(CBoard,Player,Number):-%get Next Plays, and go evaluate the board according to forks
		extractCBoard(CBoard,Board,_),
		%should I remove last play from the Board first ?
		%Set Last Play to none
		setSquareValue(CBoard,none,BoardMinusOnePlay),
		getNextPlays(BoardMinusOnePlay,SqrList),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	alphaBeta(CBoard,_,_,Player,0,Heuristic,CBoard):-%go evaluate board
		opponentPlayer(Player,Opponent),
		evaluateBoard(CBoard,Opponent,Heuristic),	!.
...
%			AlphaBetaAux
	...
	alphaBetaAux([Head|Tail],Alpha,Beta,TmpCBoard,oh,Depth,NewBeta,NewCBoard):-%Same as depth isn't zero with ex
		alphaBeta(Head,Alpha,Beta,ex,Depth,Heuristic,_),%Use the seventh Var to see the choosed board from the children ( in debugging ), or maybe to output the path ( later ).
...
/* code in AlphaBeta
	...
	evaluateBoard(CBoard,Player,Return):-%this play will make the oponent block my win which is bad ((Last Play))
		extractCBoard(CBoard,Board,Square),
		opponentPlayer(Player,Opponent),
	...
	evaluateBoard(CBoard,Player,Number):-
		evaluationFunction(CBoard,Player,Number).
		
	/*	not needed
	evaluate_Board(Board,Player,SqrList,Number):- %opponent player does fork
		opponentPlayer(Player,Opponent),
		doesFork(Board,SqrList,Opponent),
		playerValue(Opponent,OpponentValue),
		Number = OpponentValue * 800,	!.
		*/
		
%		/* code in AlphaBeta _ Unused
	evaluate_Board(Board,Player,SqrList,Number):-%does fork next time ... horizontal or vertical ... return 800 * PlayerValue
		doesFork(Board,SqrList,Player),
		playerValue(Player,Value),
		Number = Value * 800,	!.
